			...
			if (initing || saving) begin
				if (drv_change) begin
					...
					if (update[drv_sel_s]) begin
						initing <= 1;
						`init_track(drv_sel_s); /* Error: procedural assignment to a non-register sd_blk_cnt,sd_lba is not permitted, left-hand side should be reg/integer/time/genvar */
					end
					else begin
